...
export const load: PageServerLoad = async ({ locals, parent }) => {
	...
	if (familyMembersList.length === 0) {
		return { period: { start: start.toISOString(), end: end.toISOString(), label }, rankings: [] };
	}

	const memberIds = familyMembersList.map((m) => m.id);

	const rows = await db
		.select({ memberId: choreCompletions.memberId, total: sum(choreCompletions.coinsAwarded) })
		.from(choreCompletions)
		.where(and(inArray(choreCompletions.memberId, memberIds), gte(choreCompletions.completedAt, start.toISOString()), lte(choreCompletions.completedAt, end.toISOString())))
		.groupBy(choreCompletions.memberId);

	const totalsMap = new Map<string, number>(rows.map((r) => [r.memberId, Number(r.total ?? 0)]));
	...
	const rankings = unsorted.map((entry, index) => ({ rank: index + 1, ...entry }));
